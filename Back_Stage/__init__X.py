...
def Processor_Creation():
    ...
    def get_Yc(user_email):
        """从Firestore获取Yc变量"""
        yc_ref = db.collection('State').document(user_email)
        yc_doc = yc_ref.get()
        if yc_doc.exists:
            Yc_data = yc_doc.to_dict().get('value')
            Yc = torch.tensor(Yc_data)
            print("Yc loaded from Firestore:", Yc)
            return Yc
        else:
            print("Yc document does not exist. Returning default value.")
            return torch.tensor([0, 0])

    def update_Yc(user_email, Yc):
        """更新Firestore中的Yc变量"""
        yc_ref = db.collection('State').document(user_email)
        # 将Yc值保存为列表形式（可以被Firestore序列化）
        yc_ref.set({'value': Yc.tolist()})
        print("Yc updated in Firestore:", Yc)
    ...
    def upload_file():
        ...
        if file:
            ...
            if not processing_event.is_set():
                ...
                threading.Thread(target=MER, args=(user_email,)).start()
